...
def build_list(num_sets, current_list, unused_keys):
    for key in unused_keys:
        for n in num_sets[key]:
            if len(current_list) == 0 or current_list[-1] % 100 == n // 100:
                new_list = current_list + [n]
                yield from build_list(num_sets, new_list, list(filter(lambda k: k != key, unused_keys)))
    yield current_list
...
def problem61():
    ...
    for l in build_list(num_sets, [], ['3', '4', '5', '6', '7', '8']):
